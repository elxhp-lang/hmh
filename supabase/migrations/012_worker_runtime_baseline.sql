-- Worker runtime baseline for Creative Xiaohai
-- Keeps migration chain aligned with production worker features.
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'worker_tasks_status_check') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'worker_task_items_status_check') then
...
create unique index if not exists uq_worker_task_items_task_index
  on public.worker_task_items(task_id, item_index);
